...
function initBendPage(){
  ...
  function calc(){
    ...
    $("angleRad").textContent=r3(angleRad);
    $("ossb").textContent=r3(OSSB);
    $("neutralR").textContent=r3(neutralRadius);
    $("ba").textContent=r3(BA);
    $("bd").textContent=r3(BD);
    $("flatDec").textContent=r3(flat);
    $("flatFrac").textContent=toNearestSixteenth(flat);

    const boris=$("borisFlat");
    if(boris) boris.textContent=toNearestSixteenth(flat);
...
function initTonnagePage(){
  ...
  const borisGraphic=$("borisGraphic");

  fillThicknessSelect(t_thk,6);
  fillDieOpeningSelect(t_dieOpening,3);
  ...
  function calc(){
    ...
    tpf.textContent=`${toTons(tonsPerFoot)} tons/ft`;
    total.textContent=`${toTons(totalTons)} tons`;

    if(borisAnswer){

      if(totalTons>200){
        borisAnswer.textContent=`NO — Over 200 tons (${toTons(totalTons)})`;
        borisGraphic.className="graphicPlaceholder overLimit";
      }
      else{
        borisAnswer.textContent=`YES — Under 200 tons (${toTons(totalTons)})`;
        borisGraphic.className="graphicPlaceholder underLimit";
      }
